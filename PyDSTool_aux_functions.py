...
def plot_continuation(ODE, PCargs, nmodel, keys, freepar, names, fs=[6,5], off_points=False, nrow=None, ncol=None, 
                      ...
                      showcurve=True, FPs=None, linewidth=3, normal_form_coef=False, silence=False):
...
#-------------------------------------------------------------------------------------------------------------------#
def plot_continuation_twocells(ODE, DSargs, nmodel, freepar, vfreepar, par, rpar, names, xlim, ylim, figname=False, saveeigen=True, 
                               maxstep=1e-0, minstep=1e-2, step=5e-2, keys=['N1'], silence=False):
    if silence:
        class NullDevice():
            def write(self, s):
                pass
    ...
    ODE.set(pars = {freepar: vfreepar, par: rpar} )
    fp = eliminate_redundants(pp.find_fixedpoints(ODE, n=2, maxsearch=1e+3, eps=1e-10),6)
    PCargs = PyCont_args(nmodel, freepar, 3000, saveeigen=saveeigen, maxstep=maxstep, stopAt=['B'], minstep=minstep, step=step)
    p = plot_continuation(ODE, PCargs, nmodel,['N1','D1','J1','S1'], freepar, names, FPs=[fp[0]],
                          off_points=True, fontsize=20, xlim=DSargs.pdomain[freepar], LimitPoints='BP')

    figure(figsize=(7,5))
    l = sorted(p + DSargs.pdomain[freepar])
    ...
    for i in range(len(l)-1):
        ...
        fp = eliminate_redundants(pp.find_fixedpoints(ODE, n=2, maxsearch=1e+3, eps=1e-10),6)
        print len(fp), fp
        PCargs = PyCont_args(nmodel, freepar, 50000, saveeigen=saveeigen, stopAt=['B','BP'],
                             maxstep=maxstep, minstep=minstep, step=minstep)
...
def param_sensitivity_bifurcations(DSargs, pars, freepar, var, nmodel, delta=[0.0, 0.1, -0.1], c=['k', 'b', 'r'], 
                                   ylim=None, save_fig=None, maxstep=1e+1, minstep=1e-3, step=5e+0, silence=False):
...
#-------------------------------------------------------------------------------------------------------------------#
def plot_phasediagram(ODE, freepar, r_var, r, xlabel, ylabel, xlim, ylim, nmodel, names, fig_name=False, silent=False, fng_effect=False):
    if silent:
        class NullDevice():
            def write(self, s):
                pass
    ...
    for i in r:
        if fng_effect:
            ODE.set(pars = {'ldf': i, 'ljf': 1.0/float(i)} )
        else:
                    ODE.set(pars = {r_var: i} )
        fp = eliminate_redundants(pp.find_fixedpoints(ODE, n=2, maxsearch=1e+4, eps=1e-12),4)
        ODE.set(ics  = fp[0])        
        PCargs = PyCont_args(nmodel, freepar, 500, saveeigen=True, maxstep=1e+2, minstep=1e-2, step=5e+1) 
        lp = plot_continuation(ODE, PCargs, nmodel,['N'], freepar, names, fs=[4,3], off_points=True, LimitPoints='LP', fontsize=12, showcurve=False)
        if len(lp) == 2:
            x += [[i,lp[0],0]]
            x += [[i,lp[1],3]]
        else:
    ...
    for i in range(4):
        if fng_effect:
            plt.plot(x[x[:,2]==i][:,0],x[x[:,2]==i][:,1]/1000, color='k')
        else:
            plt.plot(x[x[:,2]==i][:,0]/1000,x[x[:,2]==i][:,1]/1000, color='k')
    plt.xlabel(xlabel, fontsize= 18)
    plt.ylabel(ylabel, fontsize= 18)
    plt.xlim(xlim)
    plt.ylim(ylim)
    if fig_name:
        plt.savefig('./figures/'+fig_name+'.pdf', format='pdf', dpi=500)
#-------------------------------------------------------------------------------------------------------------------#